...
function History(props) {
  ...
  const handleActionBot = async (_id, actionType) => {
    ...
    if (response.data.ok) {
      await getBots();
      enqueueSnackbar(translate(response.data.d.err_code), {
        variant: "success",
      });
  ...
  const deleteHistory = async () => {
    ...
    handleOpenAlertDialog(
      translate("are_u_want_to_del_history_config"),
      "",
      "warning",
      next
    );
      ...
      dataHistory.forEach((history, index) => {
        const obj = {
          ...
          botData: (history.botData =
            typeof history.botData === "string"
              ? JSON.parse(history.botData)
              : history.botData),
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            >
              <TableRow>
                ...
                <TableCell
                  style={{ color: theme.palette.mode === "light" && "black" }}
                >
                  {translate("nickname")}
                </TableCell>
                <TableCell
  ...
  if (row.result === 4) {
    return (
      <>
        ...
        {row.total_profit !== undefined && row.total_profit !== 0
          ? `: ${row.total_profit} $`
          : ""}
      </>
    );
...
const BetType = ({ row }) => {
  if (row.betType === "NONE") {
    ...
    if (
      row.message === "bot_take_target" ||
      row.message === "total_profit_taking" ||
      row.message === "total_stop_loss"
    )
      return (
        <IconButton sx={{ bgcolor: "warning.main" }} size={"small"}>
          <Iconify
  ...
  return (
    <IconButton
      sx={{ bgcolor: row.betType === "UP" ? "success.main" : "error.main" }}
      size={"small"}
    >
      <Iconify
        icon={
...
const ExtraComponent = ({ row }) => {
  ...
  return (
    <TableCell colSpan={6}>
      ...
      <Typography>
        {translate("profit")} :{" "}
        {parseFloat(parseFloat(row.botData.runningData.profit).toFixed(2))} $
      </Typography>
      {(row.botData.botInfo.child_loss && (
        <Typography>
          {translate("child_profit")} :{" "}
          {parseFloat(row.botData.runningData.child_profit.toFixed(2))}$
        </Typography>
      )) ||
      ...
      <Typography>
        {translate("volume")} :{" "}
        {parseFloat(parseFloat(row.botData.runningData.volume).toFixed(2))} $
      </Typography>
      {row.botData.runningData.lastMethodName && (
        <Typography>
          {translate("method")} :{" "}
          {translate(row.botData.runningData.lastMethodName)}
        </Typography>
      )}
      {row.botData.runningData.chain_method &&
        row.botData.runningData.chain_method.isRunning && (
          <Typography>
            {translate("satisfaction_condition")} :{" "}
            {row.botData.runningData.chain_method.currentChain}
          </Typography>
        )}

      <Typography>
        {translate("capital_management")}{" "}
        {row.botData.botInfo.capital_management_type === 0 &&
          translate("martingale")}{" "}
        {row.botData.botInfo.capital_management_type === 1 &&
          translate("fibo1step")}{" "}
        {row.botData.botInfo.capital_management_type === 2 &&
          translate("fibo2step")}{" "}
        {row.botData.botInfo.capital_management_type === 3 &&
          translate("victor1")}{" "}
        {row.botData.botInfo.capital_management_type === 4 &&
          translate("victor2")}{" "}
        {row.botData.botInfo.capital_management_type === 5 &&
          translate("victor3")}{" "}
        {row.botData.botInfo.capital_management_type === 6 &&
          translate("money_custom")}{" "}
        {row.botData.botInfo.capital_management
          .split("\n")
          .map((capital, key) => (
  ...
  return (
    <>
      ...
      >
        ...
        <span style={{ paddingLeft: 1, paddingRight: 10 }}>
          {translate("actions")}
        </span>
      </Button>
