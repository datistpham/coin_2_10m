...
function ConfigurationList() {
  ...
  const handleDeleteConfiguration = async (_id) => {
    ...
    handleOpenAlertDialog(
      translate("are_u_want_to_del_config"),
      "",
      "info",
      next
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 0.8,
      renderCell: (cellValues) => {
        const findNickname = exchangeAccounts.find(
          (a) => a._id === cellValues.row.linkAccountId
        );
...

    {
      ...
      renderCell: (cellValues) => {
        ...
        return (
          ...
          >
            {current.profit > 0
              ? `+${parseFloat(current.profit.toFixed(2))}`
              : parseFloat(current.profit.toFixed(2))}{" "}
            $
          </Label>
        );
  ...
  const handleStartSelectedItem = async () => {
    ...
    enqueueSnackbar(
      `${translate("bot_start")} ${current.count} ${translate(
        "configuration"
      )}`,
      {
        variant: "success",
  ...
  const handleStopSelectedItem = async () => {
    ...
    enqueueSnackbar(
      `${translate("bot_stop")} ${current.count} ${translate("configuration")}`,
      {
        variant: "success",
      }
    );
  ...
  const handleDeleteSelectedItem = async () => {
    const next = async () => {
      ...
      enqueueSnackbar(
        `${translate("successful_delete")} ${current.count} ${translate(
          "configuration"
        )}`,
        {
          variant: "success",
