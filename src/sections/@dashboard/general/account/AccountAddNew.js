...
const AccessAPIKey = ({ setOpenRegister }) => {
  ...
  const onSubmit = async (e) => {
    try {
      const response = await API_EXCHANGE.postLinkAccountToken({
        api_key: e.token,
      });
      ...
      enqueueSnackbar(
        translate(response.data.errorType || response.data.message),
        { variant: "error" }
      );
...
const AccessEmailPassword = ({ setOpenRegister }) => {
  ...
  const CreateAPISchema = Yup.object().shape({
    email: Yup.string()
      ...
      .email(translate("email_invalidate")),
  ...
  const onSubmit = async (e) => {
    try {
      const response = e.token2FA
        ? await API_EXCHANGE.postLinkAccount2FA(e)
        : await API_EXCHANGE.postLinkAccount(e);
      if (response.status === 200) {
        if (response.data.success) {
          ...
          if (response.data.login2FARequired) {
            setRequire2fa(true);
            setValue("token2FA", response.data.login2FAToken);
        ...
        {require2fa ? (
          ...
          <>
            ...
            <Grid item xs={12} md={12} marginBottom={2}>
              <RHFTextField
                name="email"
                label={translate("exchange_login_email")}
                size="small"
              />
            </Grid>
            <Grid item xs={12} md={12} marginBottom={2}>
