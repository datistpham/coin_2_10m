...
const Transactions = ({ linkAccountId }) => {
  ...
  const getTransactions = async () => {
    const response = await API_WALLET.spotTransactions(
      linkAccountId,
      page,
      total
    );
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {rows.map((row, index) => (
                ...
                >
                  <TableCell>
                    {format(new Date(row.transactionTime), "HH:mm:ss dd/MM")}
                  </TableCell>
                  <TableCell>
                    <Amount row={row} />
                  </TableCell>
...
const Status = ({ row }) => {
  ...
  return (
    <Iconify sx={{ color: current.color, width: 60 }} icon={current.status} />
  );
};
