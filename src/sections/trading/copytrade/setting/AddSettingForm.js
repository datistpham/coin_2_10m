...
function AddSettingForm(props) {
  ...
  const CreateSettingSchema = Yup.object().shape({
    ...
    brokerUserName: Yup.string().required(
      translate("Vui lòng nhập tên tài khoản chuyên gia")
    ),
    moneyPerOrder: Yup.number(translate("must_enter_number")).required(
      translate("Vui lòng nhập giá trị 1 lệnh")
    ),
    stopLossTarget: Yup.number(translate("must_enter_number")).required(
      translate("Vui lòng nhập mục tiêu cắt lỗ")
    ),
    takeProfitTarget: Yup.number(translate("must_enter_number")).required(
      translate("Vui lòng nhập mục tiêu chốt lãi")
    ),
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.status === 200) {
        ...
        enqueueSnackbar(translate(response.data.err_code), {
          variant: "error",
        });
        ...
        <CardContent>
          {isLoading ? (
            ...
            >
              ...
              <RHFTextField
                name="moneyPerOrder"
                label={translate("money_per_order")}
                size="small"
              />
              <br />
              <RHFTextField
                name="takeProfitTarget"
                label={translate("take_profit_target")}
                size="small"
              />
              <br />
              <RHFTextField
                name="stopLossTarget"
                label={translate("stop_loss_target")}
                size="small"
              />
              <br />
              ...
              <RHFSwitch
                ...
                label={translate("allow_notifications")}
                labelPlacement="end"
              />
              ...
              <Stack spacing={1} direction={{ xs: "column", sm: "column" }}>
                <LoadingButton
                  ...
                  startIcon={
                    <Iconify
                      icon={isEdit ? "eva:edit-2-fill" : "carbon:add-filled"}
                    />
                  }
                >
                  {isEdit
                    ? translate("edit_configuration")
                    : translate("add_configuration")}
                </LoadingButton>
              </Stack>
            </Stack>
          )}
