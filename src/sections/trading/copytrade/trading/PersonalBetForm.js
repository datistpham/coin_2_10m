...
function PersonalBetForm() {
  ...
  const exchangeAccounts = useSelector((state) => {
    console.log(state);
    return state.exchangeAccountsLogined;
  });

  const BetSchema = Yup.object().shape({
    accountType: Yup.string().required(
      translate("this_is_required_information")
    ),
    marginDense: Yup.number(translate("must_enter_number")).required(
      translate("this_is_required_information")
    ),
          ...
          >
            ...
            >
              {exchangeAccounts ? (
                ...
                >
                  <div style={{ display: "flex" }}>
                    {translate("not_affiliate_account_yet")}
                  </div>
                </MenuItem>
              )}
            </RHFTextField>
            ...
            <RHFTextField
              name="marginDense"
              label={translate("bet_amount")}
              size="small"
            />

            <Typography
              variant="caption"
              sx={{ textAlign: "center", color: "orange", fontSize: "1em" }}
            >
              {boPrice && boPrice.isBetSession
                ? `${translate("session")} ${
                    boPrice.session ? boPrice.session : "0"
                  } : ${translate("please_trade")} (${
                    boPrice.order ? boPrice.order : "0"
                  }s)`
                : `${translate("session")} ${
                    boPrice.session ? boPrice.session : "0"
                  } :  ${translate("waiting_result")}  (${
                    boPrice.order ? boPrice.order : "0"
                  }s)`}
            </Typography>
            <Stack spacing={1}>
