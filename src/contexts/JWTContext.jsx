...
const AuthContext = createContext({
  ...
  recover_password: ()=> Promise.resolve()
});
...
function AuthProvider2({ children }) {
  ...
  const forgot_password = async (username) => {
    const response = await API_AUTH.getRecoveryCodeForgotPass(
      username,
    );
    const { accessToken, refreshToken, user } = response.data;
    setSession(accessToken, refreshToken);
  ...
  const recover_password = async (username, code, newPassword) => {
    const response = await API_AUTH.postResetPassword(
